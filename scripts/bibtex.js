...
function BibtexParser() {
    ...
    this.bibtex = function() {
        ...
        while (this.tryMatch("@")) {
            start = this.pos;
			code_index = pdf_index = project_index = -1;
			...
			// Simple hack assuming pdf, code, project are located at the end of each entry 
			tend = end;
			pdf_index = this.input.substring(start, end).indexOf('pdf');
			if (pdf_index != -1)
				tend = pdf_index;
			code_index = this.input.substring(start, end).indexOf('code');
			if (code_index != -1 && code_index < tend)
				tend = code_index;
			project_index = this.input.substring(start, end).indexOf('project');
			if (project_index != -1 && project_index < tend)
				tend = project_index;						
			if (tend != -1)
				this.entries[this.currentEntry]["BIBTEXRAW"] = this.entries[this.currentEntry]["BIBTEXRAW"].substring(0,tend);
        }
    }
}
